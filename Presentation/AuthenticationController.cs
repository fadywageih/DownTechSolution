...
namespace Presentation
{
    ...
    public class AuthenticationController(IServiceManager serviceManager) : ControllerBase
    {
        ...
        [HttpPost("register")]
        public async Task<ActionResult<UserResultDto>> Register(RegisterDto schoolRegisterDto)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            }
            var result = await serviceManager.AuthenticationService.Register(schoolRegisterDto);
            return Ok(result);
        }
        ...
        public async Task<ActionResult> ResetPassword([FromBody] ResetPasswordRequestDto model)
        {
            ...
            string decodedToken;
            try{decodedToken = Uri.UnescapeDataString(model.Token);}
            catch{decodedToken = model.Token;}
            var result = await serviceManager.AuthenticationService.ResetPassword(model.Email, decodedToken, model.Password);
            if (result){return Ok(new { message = "Password has been reset successfully" }); }
            else{return BadRequest(new { error = "Invalid or expired reset token" });}
        }
    }
}